describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const buttonText = await addCartButton.getProperty('innerText');
    const buttonTextJSONVal = await buttonText.jsonValue();
    expect(buttonTextJSONVal).toBe('Remove from Cart');
  }, 2500);
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      const buttonText = await addCartButton.getProperty('innerText');
      const buttonTextJSONVal = await buttonText.jsonValue();
      if(buttonTextJSONVal != 'Remove from Cart'){
        await addCartButton.click();
      }
    }
    // Check to see if the innerText of #cart-count is 20
    const cartCount = await page.$eval('#cart-count', (elem) => elem.innerText);
  ...
  it('Checking number of items in cart on screen after reload', async () => {
    ...
    for(let i = 0; i < prodItems.length; i++){
      ...
      const buttonText = await addCartButton.getProperty('innerText');
      const buttonTextJSONVal = await buttonText.jsonValue();
      expect(buttonTextJSONVal).toBe('Add to Cart');
    }
    // Also check to make sure that #cart-count is still 20
    const cartCount = await page.$eval('#cart-count', (elem) => elem.innerText);
    expect(cartCount).toBe('0');
